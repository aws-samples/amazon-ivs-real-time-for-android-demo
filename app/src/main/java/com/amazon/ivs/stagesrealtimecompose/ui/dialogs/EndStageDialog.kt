...
fun EndStageDialog() {
    Column(
        ...
        Text(
            text = stringResource(R.string.end_stage),
            style = RobotoPrimary,
        )
        ButtonPrimary(
            modifier = Modifier.padding(top = 32.dp, bottom = 20.dp),
            text = stringResource(R.string.end_stage_for_everyone),
            background = RedPrimary,
            textColor = WhitePrimary,
            onClick = StageHandler::endStage
        )
        ButtonPrimary(
...
@Preview
@Composable
private fun EndStageDialogPreview() {
    PreviewSurface {
        EndStageDialog()
    }
}
